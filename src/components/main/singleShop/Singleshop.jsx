...
function Singleshop() {
  const [chexols, setChexols] = useState([]);
  const { id } = useParams();
  ...
  function add(item, e) {
    ...
    let lll = chexols.find((el) => el.id === item.id);
    local = [...local, lll];
    ...
    <React.Fragment>
      {chexols
        ...
        .map((item) => {
          return (
            <section key={item.id}>
              <h3 className="type-item">{item[lang]}</h3>
              <div className="chexol-box">
                <div className="chexol-single-item">
                  <img
                ...
                <div className="chexol-single-item-right">
                  <h4 className="type-item characters">
                    Характеристики и описание
                  </h4>
                  <p className="character-desc">{item.desc}</p>
                  <div className="desc-box">
                    <p className="chexol-desc product-title">{item[lang]}:</p>
